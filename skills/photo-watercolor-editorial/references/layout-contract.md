...
Resolve this contract after [Subject Lock, Relational Support, and Residual Trace](subject-salience-contract.md). It governs four independent decisions: composition geometry, design intervention, primary completeness, and exact aspect ratio. It never gives the full source frame primary protection.
...
- indispensable support, one relational-support family, and residual traces within their budgets;
...
Support may remain incomplete once its relation reads. Relational support receives no completeness or safe-area protection. Structural and atmospheric traces may bleed.
...
- Do not infer a custom ratio from the source photograph, subject type, template, or style reference. In `auto`, do not select any ratio other than `3:5` or `5:3`.
...
title_fields: [primary corner / distinct fallback corner / protected exclusions]
```

## 5. Reserve title fields after geometry

Choose one primary corner slot from `top-left`, `top-right`, `bottom-left`, or `bottom-right` only after the protected subject, meaningful interval, decisive terminals, indispensable support, and any embedded-photo field are known. Choose a distinct fallback slot that is also clear of those regions.
...
- In `artifact-full`, compile the primary slot only as calm, empty open paper with an even light value. Do not expose its later use to ImageGen. The local finalizer uses the primary slot first and the fallback only after a recoverable fit failure.
...
- Never place a title over a protected face, contact, meaningful gap, decisive silhouette, embedded source photo, or active focal contrast.
