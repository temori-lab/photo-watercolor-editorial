...
- Separate **evidence confidence** from **subject importance**. A reliable visible object may still be removable context. Resolve [Subject Lock and Residual Trace](subject-salience-contract.md) before protecting any element.
...
Use in `poster-only` when a new scale, placement, sparse setting, or structural compression improves the poster and visible evidence supports it. Select `standard-editorial` for restrained improvement or `poster-rebuild` for a major redesign of framing, overall scale and placement, negative space, tonal hierarchy, and support or trace arrangement. Under `source-locked`, transform only the selected primary relational group as one unit; under `editorial-recompose`, change independent primary placement and scale within evidence boundaries. Omit uncertain detail and nonessential scene objects cleanly rather than blurring, completing, or reconstructing them.
...
primary: [minimal literal subject set + recognition cues]
support: [minimum event-bearing support + simplified form]
relational_support: [none / one recognizable family + qualitative relation + compressed cues]
generic_without_context: [yes/no + short reason]
structural_trace: [none / up to three marks + function]
atmospheric_trace: [none / one broad wash or sparse rhythm + function]
drawable_scope: [positive content passed to image generation]
full_scene_reconstruction: prohibited
strongest supported relation: [relation]
